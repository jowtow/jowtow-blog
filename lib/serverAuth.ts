...
export async function verifyAdminAuth(request: NextRequest): Promise<AdminAuthResult> {
  ...
  try {
    ...
    if (!response.ok) {
      return { ok: false, status: 401, error: 'Unauthorized - Invalid identity token' };
    }
    ...
    if (!email) {
      return { ok: false, status: 401, error: 'Unauthorized - Email missing from identity profile' };
    }

    if (!allowlist.includes(email)) {
      return { ok: false, status: 403, error: 'Forbidden - User is not an admin' };
    }
